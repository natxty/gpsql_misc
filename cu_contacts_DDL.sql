...
WHILE EXISTS ( SELECT * FROM @T WHERE Processed = 0)
BEGIN   --Get the Id of the first row
       	SELECT Top 1 @Id = contact_id FROM @T WHERE Processed= 0
                 
        --do processing here
        
        UPDATE @T SET Processed = 1 WHERE contact_id = @Id
END;

SELECT * FROM @T WHERE (locationname IS NULL OR addressline1 IS NULL OR phone1 IS NULL OR email IS NULL) ORDER BY ref_id ASC;