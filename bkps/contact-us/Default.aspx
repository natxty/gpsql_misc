...
function ContactUsApp(settings) {
	...
	App.drillDownView = Ember.View.create({
	  ...
	  */
	  	...
	  	switch(name) {
	  		...
	  		case 'itype':
	  			...
	  			*/
	  			...
	  			if (value == 4) { // Locations
		  			/*
		  				Populate the Location 
		  			*/
			  		var data = {
			  			'action'					: 'get_informationtype_to_location',
			  			'informationtype_id'		: value,
				  		'us_site'					: App.us_site,
				  		'gl_site'					: App.gl_site
			  		}
	  		...
	  		case 'pstype':
	  			...
	  			*/
	  			...
	  			if ( jQuery.inArray(value, [11,16]) == -1 ) { // Transplant and Transfusion Medicine + Coagulation Products
		  			/*
		  				Populate the Location 
		  			*/
			  		var data = {
			  			'action'					: 'get_location_shortcut',
			  			'informationtype_id'		: App.drillDownView.get('itype_id'),
			  			'productservicetype_id'		: value,
				  		'us_site'					: App.us_site,
				  		'gl_site'					: App.gl_site		  			
		  			...
		  			jQuery.get('/contact-us/cu_controller.aspx', data, function(html) {
		  				_this.set('locat', html);
		  				setOption();
		  				//Ember.run.next(_this, 'chooseFirstLocation');
		  			});		  			  			
				}	  			
	  		break;
	  		case 'psline':
	  			...
	  			*/
	  			...
	  			if ( jQuery.inArray(value, [34,35,36]) == -1 ) { // LIFECODES Transplant + LIFECODES Transfusion + LIFECODES Coagulation Products
		  			/*
		  				Populate the Location 
		  			*/
			  		var data = {
	  		...
	  		case 'psname':
	  			...
	  			*/
	  			...
	  			jQuery.get('/contact-us/cu_controller.aspx', data, function(html) {
	  				_this.set('locat', html);
	  				setOption()
	  				/* See how many elements we have, if only 1 then select it and submit our form again */
					Ember.run.next(_this, 'chooseFirstLocation');	
	  			});
	  ...
	  chooseFirstLocation:function() {
			if ( jQuery('#locat option').length == 2 ) {
				jQuery('#locat').val(jQuery('#locat option:last').val());
	...
	App.locationView = Ember.View.create({
	  ...
	  get_location_info: function() {
			...
			jQuery.get('/contact-us/cu_controller.aspx', data, function(html) {
				...
				// Do some stuff to make our google maps show up
				Ember.run.next(_this, 'googleMaps');
			});
	  }.observes('App.drillDownView.locat_id'),
	  
	  googleMaps:	function() {
	  		var $facInfo = jQuery('div.contact-us-facility-info');
	  		if ( $facInfo.length > 1 ) {
	  			// Multiple results, so we'll add in a link
	  			$facInfo.each(function(i,el) {
	  				var address = jQuery(el).find('div.contact-us-facility-address').text();
	  				jQuery(el).append('<a href="#" class="show-map">Show map</a>');
	  				jQuery(el).find("a.show-map").click(function() {
	  					loadMapFromAddress(address);
	  					return false;
	  				});
	  			})
	  		} else {
	  			// Just display one address
	  			var address = $facInfo.find('div.contact-us-facility-address').text();
	  			loadMapFromAddress(address);
	  		}
	  },
		...
		*/
		...
		var reset = function(li) {
			jQuery.each(li, function(key,value) {
				...
				App.drillDownView.set(value+'_id',0);
			});
...
<script src="http://maps.google.com/maps?file=api&amp;v=2&amp;sensor=true&amp;key=AIzaSyCkO0zgqvFx7HZIAvo_TSl_8nRgwjGF3Hc" type="text/javascript"></script>
<script type="text/javascript">
//<![CDATA[
